...
export class SpecializedAnalyzers {
  ...
  static async auditWebsite(websiteUrl: string): Promise<AnalysisResult> {
    ...
    const uniqueTrackers = Array.from(
      ...
      riskLevel:
        siteEvents.find(e => e.domain === domain)?.riskLevel || 'unknown',
    }));

    const thirdPartyCount = uniqueTrackers.filter(
      t => !t.domain.includes(domain)
    ).length;
    const thirdPartyPercentage = Math.round(
      (thirdPartyCount / uniqueTrackers.length) * 100
    );
    ...
    const recommendations = [
      ...
      ...this.generateWebsiteRecommendations(
        uniqueTrackers,
  ...
  static async analyzeTimeline(
    ...
    const lowestDay = dailyEntries.reduce(
    ...
    const anomalies = dailyEntries
      ...
      .map(([date, count]) => ({
        ...
        eventCount: count,
        cause:
          count > dailyAverage * 3
            ? 'Possible news browsing session'
            : undefined,
      }));
  ...
  private static generateTrackerRecommendations(
    ...
    if (
      trackerInfo.riskLevel === 'critical' ||
      trackerInfo.riskLevel === 'high'
    ) {
      recommendations.push(
        `Block ${trackerInfo.name} using uBlock Origin or similar ad blocker.`
      );
    }

    if (trackerInfo.sites.length > 5) {
      recommendations.push(
        `This tracker appears on many sites (${trackerInfo.sites.length}). Consider using Privacy Badger for intelligent blocking.`
      );
    }

    if (trackerInfo.trackingMethods.includes('canvas-fingerprint')) {
      recommendations.push(
        'This tracker uses canvas fingerprinting. Use Firefox with privacy.resistFingerprinting enabled.'
      );
    }
  ...
  private static generateWebsiteRecommendations(
    ...
    if (thirdPartyPercentage > 80) {
      recommendations.push(
        `${thirdPartyPercentage}% of trackers are third-party. This site shares data extensively.`
      );
    }

    if (uniqueTrackers.length > 15) {
      recommendations.push(
        `Excessive tracking detected (${uniqueTrackers.length} trackers). Consider using this site with an ad blocker.`
      );
    }

    const criticalTrackers = uniqueTrackers.filter(
      t => t.riskLevel === 'critical'
    );
    if (criticalTrackers.length > 0) {
      recommendations.push(
        `${criticalTrackers.length} critical-risk trackers detected. Use caution on this site.`
      );
    }
  ...
  private static generateTimelineRecommendations(
    ...
    if (timelineData.anomalies.length > 0) {
      recommendations.push(
        `${timelineData.anomalies.length} tracking spikes detected. Consider batching similar browsing activities.`
      );
    }

    const peakHour = timelineData.hourlyPatterns.reduce((max, curr) =>
      curr.events > max.events ? curr : max
    );
    if (peakHour.events > timelineData.dailyAverage * 0.3) {
      recommendations.push(
        `Peak tracking at ${peakHour.hour}:00. Consider using privacy mode during heavy browsing.`
      );
    }
