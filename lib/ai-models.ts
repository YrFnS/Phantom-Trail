...
export const DEFAULT_MODEL = AI_MODELS.find(m => m.category === 'free')?.id || AI_MODELS[0].id;

// Fallback model (first fast model)
export const FALLBACK_MODEL = AI_MODELS.find(m => m.category === 'fast')?.id || AI_MODELS[1].id;
