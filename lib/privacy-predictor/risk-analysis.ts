...
export class RiskAnalysis {
    ...
    try {
      ...
      if (domain.includes('facebook') || domain.includes('instagram') || 
        ...
        factors.push({
          type: 'category-risk',
          impact: -20,
          description: 'Social media platforms typically have extensive tracking',
          confidence: 0.9
        });
      }
      ...
      if (domain.includes('.gov') || domain.includes('.edu')) {
        factors.push({
          type: 'category-risk',
          impact: 20,
          description: 'Government/educational sites typically have better privacy',
          confidence: 0.8
        });
      }
  ...
  static calculateWeightedPrediction(factors: RiskFactor[]): { score: number; confidence: number } {
    if (factors.length === 0) {
    ...
    const baseScore = 70;
    const adjustedScore = baseScore + (totalConfidence > 0 ? totalImpact / totalConfidence : 0);
