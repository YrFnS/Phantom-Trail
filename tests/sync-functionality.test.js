...
async function testConflictResolver() {
  ...
  try {
    ...
    const newestWins = ConflictResolver.resolveConflict(
      mockConflict,
      'newest-wins'
    );
    console.log('Newest wins result:', newestWins);

    const localWins = ConflictResolver.resolveConflict(
      mockConflict,
      'local-wins'
    );
...
async function runAllSyncTests() {
  ...
  console.log(
    'Conflict Resolver:',
    results.conflictResolver ? '✅ PASS' : '❌ FAIL'
  );
  console.log(
    'Storage Manager:',
    results.storageManager ? '✅ PASS' : '❌ FAIL'
  );
