...
async function testWebsiteCategorization() {
  ...
  const { WebsiteCategorization } = await import('./lib/website-categorization.js');
...
async function testPrivacyComparison() {
  ...
  try {
    ...
    const categoryComparison = await PrivacyComparison.compareToCategory('amazon.com');
    console.log('Category comparison result:', categoryComparison);
    
    // Test user comparison
    console.log('2. Testing user comparison...');
    const userComparison = await PrivacyComparison.compareToUserAverage('amazon.com');
    ...
    const similarSites = await PrivacyComparison.compareSimilarSites('amazon.com');
    console.log('Similar sites comparison result:', similarSites);
    
    // Test comprehensive insights
    console.log('4. Testing comprehensive insights...');
    const insights = await PrivacyComparison.generateComparisonInsights('amazon.com');
...
async function testDifferentSiteTypes() {
  ...
  for (const site of testSites) {
    try {
      ...
      const insights = await PrivacyComparison.generateComparisonInsights(site.domain);
      console.log(`  Overall Insight: ${insights.overallInsight}`);
      console.log(`  Trust Level: ${insights.trustLevel}`);
      console.log(`  Category Percentile: ${insights.categoryComparison.percentile}`);
...
if (typeof window !== 'undefined') {
  ...
  console.log('Privacy comparison tests loaded. Run window.testPrivacyComparison.runAllComparisonTests() to test.');
}
