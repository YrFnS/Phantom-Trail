...
async function testPrivacyPredictor() {
  ...
  try {
    ...
    const prediction = await PrivacyPredictor.predictPrivacyImpact(
      'https://facebook.com'
    );
    ...
    const linkAnalysis = await PrivacyPredictor.analyzeLink(
      'https://facebook.com',
      context
    );
    ...
    const confidence =
      await PrivacyPredictor.getPredictionConfidence('https://google.com');
    ...
    const recommendations =
      await PrivacyPredictor.generateRecommendations(prediction);
...
async function testSiteIntelligence() {
  ...
  try {
    ...
    const behaviorPrediction =
      await SiteIntelligence.predictSiteBehavior('facebook.com');
...
async function testPredictionAccuracy() {
  ...
  try {
    ...
    for (const testCase of testCases) {
      ...
      const isAccurate =
        prediction.predictedScore >= testCase.expectedRange[0] &&
        prediction.predictedScore <= testCase.expectedRange[1];

      console.log(
        `${testCase.url}: ${prediction.predictedScore} (expected ${testCase.expectedRange[0]}-${testCase.expectedRange[1]}) - ${isAccurate ? '✅' : '❌'}`
      );
    ...
    console.log(
      `Prediction accuracy: ${accuracy}% (${accurateCount}/${testCases.length})`
    );

    if (accuracy < 60) {
      console.warn(
        '⚠️ Prediction accuracy below 60%, model may need improvement'
      );
    }
...
async function testPredictionPerformance() {
  ...
  try {
    ...
    console.log(
      `Batch prediction time: ${batchTime.toFixed(2)}ms (avg: ${avgTime.toFixed(2)}ms per prediction)`
    );
...
async function runAllPredictionTests() {
  ...
  console.log(
    'Privacy Predictor:',
    results.privacyPredictor ? '✅ PASS' : '❌ FAIL'
  );
  console.log(
    'Site Intelligence:',
    results.siteIntelligence ? '✅ PASS' : '❌ FAIL'
  );
  console.log(
    'Prediction Accuracy:',
    results.predictionAccuracy ? '✅ PASS' : '❌ FAIL'
  );
  console.log(
    'Prediction Performance:',
    results.predictionPerformance ? '✅ PASS' : '❌ FAIL'
  );
  console.log(
    'Prediction Components:',
    results.predictionComponents ? '✅ PASS' : '❌ FAIL'
  );
  console.log(
    'Link Hover Functionality:',
    results.linkHoverFunctionality ? '✅ PASS' : '❌ FAIL'
  );
