...
export const SyncSettingsComponent: React.FC<SyncSettingsProps> = ({ onClose }) => {
  ...
  const handleSettingChange = async (key: keyof SyncSettings, value: boolean | string) => {
    if (!settings) return;
    ...
    <div className="p-6 max-w-md mx-auto bg-white rounded-lg shadow-lg">
      ...
      <div className="mb-6 p-4 bg-gray-50 rounded-lg">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className={`text-sm ${status ? getStatusColor(status.status) : 'text-gray-600'}`}>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500">Sync your settings across all devices</p>
          </div>
          <button
      ...
      {settings.enabled && (
        <>
          {/* Sync Data Types */}
          <div className="mb-6">
            <h3 className="text-sm font-medium text-gray-900 mb-3">What to Sync</h3>
            ...
            <div className="space-y-3">
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => handleSettingChange('syncPrivacySettings', e.target.checked)}
                  className="h-4 w-4 text-blue-600 rounded border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-700">Privacy Settings</span>
              </label>
              ...
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => handleSettingChange('syncTrustedSites', e.target.checked)}
                  className="h-4 w-4 text-blue-600 rounded border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-700">Trusted Sites</span>
              </label>
              ...
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => handleSettingChange('syncPrivacyGoals', e.target.checked)}
                  className="h-4 w-4 text-blue-600 rounded border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-700">Privacy Goals</span>
              </label>
              ...
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => handleSettingChange('syncExportSchedules', e.target.checked)}
                  className="h-4 w-4 text-blue-600 rounded border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-700">Export Schedules</span>
              </label>
            </div>
          </div>
          ...
          {/* Conflict Resolution */}
          <div className="mb-6">
            <h3 className="text-sm font-medium text-gray-900 mb-3">Conflict Resolution</h3>
            <select
          ...
          <div className="flex gap-2">
            ...
            >
              {isLoading || status?.status === 'syncing' ? 'Syncing...' : 'Sync Now'}
            </button>
          </div>
        </>
      )}
      ...
      <div className="mt-6 p-3 bg-blue-50 rounded-md">
        <p className="text-xs text-blue-700">
          <strong>Privacy Note:</strong> Your data is synced through Chrome&apos;s secure sync service. 
          Sensitive information like tracking events and API keys are never synced.
        </p>
      </div>
    </div>
  );
